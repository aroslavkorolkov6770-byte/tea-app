...
// Исправлено: пути через @/ гарантируют отсутствие ошибок
    ...
    const fetchDayTea = async () => {
      try {
        const { data, error } = await supabase
          ...
          .eq('isDayTea', true)
          .limit(1)
          .maybeSingle(); // maybeSingle лучше, чем single, так как не выкидывает ошибку если пусто
        
        if (data) setDayTea(data);
      } catch (err) {
        console.log("Чай дня еще не назначен");
      }
    };
    fetchDayTea();
  }, []);
