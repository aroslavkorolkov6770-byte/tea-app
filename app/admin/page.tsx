...
export default function AdminPage() {
  ...
  const [formData, setFormData] = useState({ name: '', type: 'Зеленый', summary: '', isDayTea: false });
  ...
  const saveTea = async () => {
    ...
    else { setShowForm(false); fetchTeas(); }
  };
        ...
        {showForm && (
          <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.9)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 } as any}>
            <div style={{ background: '#161816', padding: '40px', borderRadius: '30px', width: '350px' } as any}>
              ...
              <div onClick={saveTea} style={{ background: '#4CAF50', color: '#000', padding: '15px', borderRadius: '15px', textAlign: 'center', fontWeight: 'bold', cursor: 'pointer' } as any}>СОХРАНИТЬ</div>
            </div>
          </div>
        )}
      </main>
