...
export default function SearchPage() {
  ...
  const handleSave = () => {
    ...
    setShowForm(false);
    setEditingId(null);
    setFormData({ name: '', type: 'Чай', category: 'Зеленый', strength: 'Мягкий', summary: '', desc: '', img: '', isDayTea: false });
  };
    ...
    <div style={{ backgroundColor: '#0d0f0d', minHeight: '100vh', color: '#e0e0e0', userSelect: 'none', fontFamily: 'Inter, sans-serif' }}>
      ...
      <main style={{ maxWidth: '1200px', margin: '0 auto', padding: '120px 20px', display: 'grid', gridTemplateColumns: isAdmin ? '1fr 340px' : '1fr', gap: '40px' }}>
        ...
        <section>
          ...
          {/* ПОДРАЗДЕЛЫ КРЕПОСТИ (ТЕПЕРЬ ДЛЯ ВСЕХ) */}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                  {isAdmin && (
                    <div style={{ display: 'flex', gap: '10px' }}>
                      <span onClick={(e) => { e.stopPropagation(); setEditingId(p.id); setFormData(p); setShowForm(true); }} style={{ color: '#4CAF50' }}>✎</span>
                      <span onClick={(e) => { e.stopPropagation(); deleteProduct(p.id); }} style={{ color: '#ff7675' }}>✕</span>
                    </div>
                  )}
        ...
        {isAdmin && (
          <aside style={{ position: 'sticky', top: '120px' }}>
            <div style={adminSidebar}>
              ...
              <button onClick={handleSave} style={saveBtn}>+ СОХРАНИТЬ ПРОДУКТ</button>
              <p style={{ fontSize: '11px', color: '#333', textAlign: 'center', marginTop: '15px' }}>Для редактирования деталей используйте ✎ на карточках.</p>
            </div>
          </aside>
        )}
        ...
        {showQuiz && selectedTea && (
          <div style={{ ...modalOverlay, zIndex: 30000 }}>
            <div style={modalContent}>
              ...
              {(selectedTea.quiz || [{q: "Профиль продукта понятен?", o: ["Да", "Нет"], c: 0}]).map((q, idx) => (
              ...
              <div onClick={() => setShowQuiz(false)} style={{ textAlign: 'center', marginTop: '20px', color: '#666', cursor: 'pointer', fontWeight: 'bold' }}>ЗАВЕРШИТЬ</div>
            </div>
          </div>
        )}

        {/* МОДАЛКА РЕДАКТИРОВАНИЯ (ДЕТАЛЬНАЯ) */}
        {showForm && editingId && (
          <div style={modalOverlay}>
            <div style={{ ...modalContent, maxWidth: '500px' }}>
              <h2 style={{ textAlign: 'center', marginBottom: '25px' }}>ПОЛНЫЙ РЕДАКТОР</h2>
              <input style={adminIn} placeholder="Название" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
              <textarea style={{ ...adminIn, height: '100px' }} placeholder="Описание" value={formData.desc} onChange={e => setFormData({...formData, desc: e.target.value})} />
              <input style={adminIn} placeholder="Регион" value={formData.region} onChange={e => setFormData({...formData, region: e.target.value})} />
              <input style={adminIn} placeholder="Советы" value={formData.advice} onChange={e => setFormData({...formData, advice: e.target.value})} />
...
const strengthFilterRow = { background: '#121412', padding: '12px', borderRadius: '20px', border: '1px solid #222', marginBottom: '25px', display: 'flex', gap: '10px', overflowX: 'auto' as any };
...
const infoTag = { color: '#4CAF50', fontSize: '11px', fontWeight: '900' as any, marginBottom: '8px', letterSpacing: '1px' };
...
const modalOverlay = { position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.96)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 20000, padding: '20px' } as any;
const modalContent = { background: '#111', padding: '40px', borderRadius: '40px', width: '100%', border: '1px solid #222', maxHeight: '90vh', overflowY: 'auto' } as any;